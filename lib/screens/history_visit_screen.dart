...
class _HistoryVisitScreenState extends State<HistoryVisitScreen> {
  ...
  Future<void> fetchKunjunganData() async {
    ...
    final url = Uri.parse('https://fakelocation.warungkode.com/api/kunjungan/history/$userId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          kunjunganList = dataList.map((item) => Kunjungan.fromJson(item)).toList();
          isLoading = false;
        });
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (kunjungan.fotoKnj != null && kunjungan.fotoKnj!.isNotEmpty)
                      ClipRRect(
                    ...
                    // Informasi Detail
                    _buildDetailItem(Icons.calendar_today, 'Tanggal', kunjungan.tglKnj),
                    _buildDetailItem(Icons.access_time, 'Jam', kunjungan.jamDariCreatedAt),
                    _buildDetailItem(Icons.business, 'Project', kunjungan.namaPro),
                    _buildDetailItem(Icons.abc, 'Nama', kunjungan.namaKnj),
                    _buildDetailItem(Icons.location_on, 'Lokasi', kunjungan.lokasiKnj),
                    _buildDetailItem(Icons.pin_drop, 'LatLong', kunjungan.latlongKnj),
                    _buildDetailItem(Icons.work, 'Pekerjaan', kunjungan.pekerjaanKnj),
                    _buildDetailItem(Icons.category, 'Kategori', kunjungan.kategoriKnj),
                    _buildDetailItem(Icons.person, 'Kontak', kunjungan.kontakKnj),
                    _buildDetailItem(Icons.source, 'Sumber', kunjungan.sumberKnj),
                    _buildDetailItem(Icons.edit_note, 'Hasil', kunjungan.hasilKnj),
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        ElevatedButton.icon(
                                          onPressed: () => showDetailDialog(context, kunjungan),
                                          icon: const Icon(Icons.remove_red_eye_outlined, size: 16, color: Colors.white,),
                                          label: const Text('Detail', style: TextStyle(fontSize: 15, color: Colors.white)),
                                          style: ElevatedButton.styleFrom(
...
class Kunjungan {
  ...
  factory Kunjungan.fromJson(Map<String, dynamic> json) {
    ...
    final jam = '${createdAt.hour}:${createdAt.minute.toString().padLeft(2, '0')}';
    final DateFormat formatter = DateFormat('d MMMM y', 'id'); // Format: 17 Februari 2025

...
class VisitService {
  ...
  static const String _baseUrl = 'https://apivn.internalbkg.com/api';
  static Future<Map> submitVisit({
    ...
    try {
      ...
      var request =
          http.MultipartRequest('POST', Uri.parse('$_baseUrl/kunjungan'));
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return ({'errorSession': "Sesion Sudah Habis"});
      } else {
        throw Exception(
            'error: Terdapat Error Saat Mengirim Data ,Code : ${response.statusCode}');
      }
    } catch (e) {
  ...
  static Future<List<DropdownItem>> fetchProjects(String isPMR) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$_baseUrl/project?ispmr=' +
            isPMR), // Sesuaikan dengan endpoint API Anda
        headers: {
          'Authorization': token,
